...
function renderCard(item) {
    ...
    return `
        <article class="protocol-card${urgent ? " is-urgent" : ""}${animationClass}" draggable="true" data-protocol-id="${escapeHtml(item.id)}" data-status="${escapeHtml(item.status)}" style="--tag-color:${escapeHtml(color)}">
...
function animateCardLayout(beforeRects) {
    if (shouldReduceMotion() || !beforeRects?.size) return;
    window.requestAnimationFrame(() => {
        dom.board.querySelectorAll("[data-protocol-id]").forEach((card) => {
            const before = beforeRects.get(String(card.dataset.protocolId || ""));
            if (!before) return;
...
function renderStatusColumnsPreservingScroll(statusList = [], options = {}) {
    ...
    animateCardLayout(options.beforeRects);
}
...
function queueProtocolAnimationCleanup() {
    ...
    state.animationCleanupTimer = window.setTimeout(() => {
        dom.board.querySelectorAll(".is-card-entering, .is-card-leaving").forEach((element) => {
            element.classList.remove("is-card-entering", "is-card-leaving");
...
async function loadBoard(options = {}) {
    ...
    const beforeRects = softRefresh ? captureCardRects(visibleStatusKeys()) : null;
    ...
    if (!state.boardReady) {
        ...
        dom.board.classList.add("is-loading", "is-soft-refreshing");
    }
    try {
        ...
        renderBoard({
            ...
            settle: softRefresh || animateResults
        });
    }
}
...
async function updateStatus(id, status) {
    ...
    if (moved) {
        renderStatusColumnsPreservingScroll(affectedStatuses, { beforeRects });
    }
    ...
    try {
        ...
        if (moved) {
            ...
            renderStatusColumnsPreservingScroll(affectedStatuses, { beforeRects: rollbackBeforeRects });
        }
        if (error.status === 409 && error.data?.protocol) {
...
function setArchivedVisibility(show) {
    ...
    state.archivedToggleTimer = window.setTimeout(() => {
        ...
        state.archivedTransition = "";
        renderActiveFilters();
        scheduleLoad(20, { animateFilter: true });
    }, 260);
}
...
function bindBoardEvents() {
    dom.board.addEventListener("click", async (event) => {
        ...
        if (open) await openProtocol(open.dataset.openProtocol);
        if (archive) {
...
function bindModalEvents() {
    ...
    dom.dialog.addEventListener("cancel", () => {
        flushPendingFieldSaves(currentId()).catch(console.error);
    });
