...
function welcomeSvg(name) {
    const icons = {
        ...
        user: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5"></circle><path d="M5 20c.7-4 3-6 7-6s6.3 2 7 6"></path></svg>',
...
function loadLocalIdentity() {
    const saved = readJson("protocolos.node.user.v1", {});
    const legacyName = String(localStorage.getItem("protocolos.userName") || "").trim();
    return {
        id: saved.id || "",
        name: String(saved.name || legacyName || "").replace(/^Usuário local$/i, "").trim(),
        color: saved.color || "#2563eb"
    };
}

function saveLocalIdentity(name) {
    const current = loadLocalIdentity();
    const colors = ["#2563eb", "#0f766e", "#b45309", "#9333ea", "#be123c"];
    const user = {
        id: current.id || localId(),
        name: String(name || "").trim(),
        color: current.color || colors[Math.floor(Math.random() * colors.length)]
    };
    writeJson("protocolos.node.user.v1", user);
    try { localStorage.setItem("protocolos.userName", user.name); } catch {}
    window.dispatchEvent(new CustomEvent("protocolos:user-identity", { detail: user }));
    return user;
}
...
function buildWelcomeModal(preferences) {
    ...
    wrapper.innerHTML = `
        ...
        <form class="protocol-welcome-card protocol-welcome-wizard" data-welcome-current-step="0">
            <header class="protocol-welcome-wizard-head">
                <div>
                    <span class="eyebrow">Primeiro acesso</span>
                    <h2 id="protocol-welcome-title">Bem-vindo aos Protocolos</h2>
                    <p>Uma configuração rápida para deixar o sistema pronto para você.</p>
                </div>
                <span class="protocol-welcome-step-count" data-welcome-step-count>1 de 3</span>
            </header>

            <div class="protocol-welcome-progress" aria-label="Progresso da configuração">
                <span class="is-active" data-welcome-progress="0"><b>1</b><small>Identificação</small></span>
                <span data-welcome-progress="1"><b>2</b><small>Aparência</small></span>
                <span data-welcome-progress="2"><b>3</b><small>Bem-estar</small></span>
            </div>

            <div class="protocol-welcome-steps">
                <section class="protocol-welcome-step is-active" data-welcome-step="0">
                    <div class="protocol-welcome-step-copy">
                        <span class="protocol-welcome-step-icon" aria-hidden="true">${welcomeSvg("user")}</span>
                        <div><h3>Como seus colegas devem identificar você?</h3><p>Este nome aparece somente quando você estiver com uma ficha aberta.</p></div>
                    </div>
                    <label class="protocol-welcome-identity protocol-welcome-identity-single">
                        <span class="protocol-welcome-identity-copy"><strong>Seu nome ou apelido</strong><small>Use um nome curto e reconhecível.</small></span>
                        <input name="user_name" type="text" maxlength="80" autocomplete="name" placeholder="Ex.: Gabriel" required>
                    </label>
                </section>

                <section class="protocol-welcome-step" data-welcome-step="1" hidden>
                    <div class="protocol-welcome-step-copy">
                        <span class="protocol-welcome-step-icon" aria-hidden="true">${welcomeSvg("system")}</span>
                        <div><h3>Escolha sua aparência</h3><p>As mudanças abaixo são aplicadas imediatamente neste navegador.</p></div>
                    </div>
                    ...
                    <div class="protocol-welcome-appearance-footer">
                        <section class="protocol-welcome-preview" data-welcome-density-preview="${preferences.density}" aria-live="polite">
                            <div><strong>Prévia da densidade</strong><small data-welcome-density-label>${densityLabel(preferences.density)}</small></div>
                            <div class="protocol-welcome-preview-card" aria-hidden="true"><span></span><span></span><span></span></div>
                        </section>
                        <label class="protocol-welcome-range">
                            <span class="protocol-welcome-range-head">
                                ...
                                <span class="protocol-welcome-range-copy"><strong>Tamanho da fonte</strong><small>Ajusta textos e componentes.</small></span>
                ...
                <section class="protocol-welcome-step" data-welcome-step="2" hidden>
                    <div class="protocol-welcome-step-copy">
                        <span class="protocol-welcome-step-icon is-health" aria-hidden="true">
                        ...
                        <div><h3>Lembretes leves de saúde</h3><p>Durante o expediente, o sistema pode sugerir pausas discretas sem interromper seu trabalho.</p></div>
                    </div>
                    <label class="protocol-welcome-check">
                        ...
                        <span><strong>Ativar lembretes leves de saúde</strong><small>Inclui visão, postura, movimento, alongamento e hidratação. A permissão de notificação será solicitada ao continuar.</small></span>
                    </label>
                    <p class="protocol-welcome-footnote">Você poderá alterar tema, fonte, densidade e lembretes a qualquer momento em <strong>Configurações</strong>.</p>
                </section>
            </div>

            <div class="protocol-welcome-actions protocol-welcome-wizard-actions">
                <button class="button button-secondary" type="button" data-welcome-back hidden>Voltar</button>
                <button class="button button-primary" type="button" data-welcome-next>Avançar</button>
                <button class="button button-primary" type="submit" data-welcome-submit hidden>Continuar</button>
            </div>
        </form>
    `;
    return wrapper;
...
function maybeShowWelcomeModal() {
    ...
    const nameInput = modal.querySelector("[name='user_name']");
    if (nameInput) nameInput.value = loadLocalIdentity().name;
    ...
    const submitButton = modal.querySelector("[data-welcome-submit]");
    let currentStep = -1;

    const showStep = (nextStep, direction = 1) => {
        const boundedStep = Math.max(0, Math.min(2, Number(nextStep) || 0));
        if (boundedStep === currentStep && modal.querySelector(`[data-welcome-step="${boundedStep}"].is-active`)) return;
        const previous = modal.querySelector(`[data-welcome-step="${currentStep}"]`);
        const next = modal.querySelector(`[data-welcome-step="${boundedStep}"]`);
        if (!next) return;
        previous?.classList.remove("is-active", "is-entering-forward", "is-entering-back");
        if (previous) previous.hidden = true;
        next.hidden = false;
        next.classList.remove("is-entering-forward", "is-entering-back");
        next.classList.add("is-active", direction >= 0 ? "is-entering-forward" : "is-entering-back");
        window.setTimeout(() => next.classList.remove("is-entering-forward", "is-entering-back"), 260);
        currentStep = boundedStep;
        form.dataset.welcomeCurrentStep = String(currentStep);
        modal.querySelector("[data-welcome-step-count]").textContent = `${currentStep + 1} de 3`;
        modal.querySelectorAll("[data-welcome-progress]").forEach((item) => {
            const step = Number(item.dataset.welcomeProgress);
        ...
        const focusTarget = next.querySelector(currentStep === 0 ? "[name='user_name']" : "input:checked, input, button");
        window.setTimeout(() => focusTarget?.focus({ preventScroll: true }), 40);
    };

    const advanceStep = () => {
        if (currentStep === 0) {
            const name = String(form.elements.user_name.value || "").trim();
            if (!name) {
                form.elements.user_name.focus();
                form.elements.user_name.reportValidity();
                return;
            }
        }
    ...
    backButton?.addEventListener("click", () => showStep(currentStep - 1, -1));
