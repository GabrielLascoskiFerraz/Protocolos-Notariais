...
const STATUSES = ["PARA_DISTRIBUIR", "EM_ANDAMENTO", "PARALISADOS", "PARA_CORRECAO", "LAVRADOS", "ARQUIVADOS"];
const PROTOCOL_COLUMNS = [
  "id", "ficha", "ato", "digitador", "apresentante", "data_apresentacao", "contato",
  "outorgantes", "outorgados", "valor_ato", "status", "observacoes", "urgente", "deletado",
  "tag_custom", "pasta_documentos", "revision", "created_at", "updated_at", "updated_by"
];
...
function createProtocolsTable(inner) {
  inner.exec(`
    CREATE TABLE protocolos (
      ...
      outorgados TEXT,
      valor_ato NUMERIC,
      status TEXT NOT NULL DEFAULT 'PARA_DISTRIBUIR' CHECK(status IN ('PARA_DISTRIBUIR','EM_ANDAMENTO','PARALISADOS','PARA_CORRECAO','LAVRADOS','ARQUIVADOS')),
      observacoes TEXT,
...
function migrateProtocolStatusConstraint(inner) {
  const schema = inner.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'protocolos'").get()?.sql || "";
  if (!schema || schema.toUpperCase().includes("PARALISADOS")) return;

  const existingColumns = new Set(inner.prepare("PRAGMA table_info(protocolos)").all().map((column) => column.name));
  const columns = PROTOCOL_COLUMNS.filter((column) => existingColumns.has(column));
  if (!columns.includes("id") || !columns.includes("status")) {
    throw new Error("A tabela protocolos não possui a estrutura mínima para a migração de status.");
  }

  const indexes = [
    "idx_protocolos_board", "idx_protocolos_updated", "idx_protocolos_ato",
    "idx_protocolos_digitador", "idx_protocolos_tag"
  ];
  inner.exec("PRAGMA foreign_keys = OFF; PRAGMA legacy_alter_table = ON;");
  try {
    inner.exec("BEGIN IMMEDIATE");
    indexes.forEach((name) => inner.exec(`DROP INDEX IF EXISTS ${name}`));
    inner.exec("ALTER TABLE protocolos RENAME TO protocolos_legacy");
    createProtocolsTable(inner);
    const columnList = columns.join(", ");
    inner.exec(`INSERT INTO protocolos (${columnList}) SELECT ${columnList} FROM protocolos_legacy`);
    inner.exec("DROP TABLE protocolos_legacy");
    inner.exec(`
    ...
    try { inner.exec("ROLLBACK"); } catch {}
    throw error;
  } finally {
    inner.exec("PRAGMA legacy_alter_table = OFF; PRAGMA foreign_keys = ON;");
  }
}
...
export function openDatabase(filename) {
  ...
  `);
  migrateProtocolStatusConstraint(inner);
  return db;
}
